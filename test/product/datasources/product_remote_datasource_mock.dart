...
class ProductRemoteDataSourceMock implements IProductDataSource {
  ...
  Future<List<ProductEntity>> getProducts() async {
    ...
    on DioException catch (e) {
      if (e.type == DioExceptionType.connectionError) {
        throw ServerException(ExceptionMessageMapper.socketError);
      }
      else if (e.type == DioExceptionType.connectionTimeout) {
        throw TimeoutException(ExceptionMessageMapper.timeoutError);
      }
      else {
        throw APIErrorException(ExceptionMessageMapper.apiError);
      }
    }

    on TimeoutException {
      throw TimeOutSearchException(ExceptionMessageMapper.timeoutError);
    }
    on SocketException {
      throw SocketException(ExceptionMessageMapper.socketError);
    }

    catch (e) {
      throw UnknownException(ExceptionMessageMapper.genericError);
    }
  }
